...
- Aún no se ejercitaron casos F-02/F-03/F-04 (variaciones), R-01 (responsive) ni E-01/E-02 (errores). Quedan como tareas para el siguiente ciclo de QA (tener en cuenta el nuevo intervalo de auto-refresh de 5 minutos).
- Nueva funcionalidad pendiente de verificación formal: agregar/quitar tickers, persistencia compartida vía `/api/tickers`, estado vacío y enlaces a Yahoo Finance. QA debe cubrirlos en la próxima ronda.

## Próximos pasos
1. Ejecutar el resto de la matriz de pruebas del `QA_PLAN.md` (incluye nuevos casos C-03 ⇢ C-08 sobre agregar/quitar tickers, persistencia y links).
...
4. Registrar resultados en una nueva sección del reporte con fecha de ejecución, símbolos utilizados y verificación multi-cliente.
